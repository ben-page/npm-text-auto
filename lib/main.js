...
if (typeof Buffer.prototype.indexOf === 'undefined') {//slow shim
...
function checkFile(filePath, stats) {
    ...
    fs.open(filePath, 'r', function (err, fd) {
        ...
        fs.read(fd, buf, 0, readLength, 0, function(err, bytesRead) {
            ...
            fs.read(fd, buf, bytesRead, stats.size - bytesRead, bytesRead, function(err, bytesRead2) {
                if (!!err)
                    return error(err);

                if (bytesRead2 !== stats.size)
...
function convertFile(filePath, stats, buf) {
    ...
    fs.writeFile(filePath, str, { encoding: 'utf8' }, function (err) {
        ...
        //console.log('converting ' + filePath + ' ' + stats.mtime);
    });
}
